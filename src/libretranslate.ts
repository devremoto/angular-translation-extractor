...
async function translateText(text: string, targetLang: string, sourceLang?: string): Promise<string> {
    ...
    async function doTranslate(target: string): Promise<string> {
        ...
        // Note: Quality is significantly lower than Google Translate
        ...
        if (response.data && response.data.translatedText && typeof response.data.translatedText === 'string') {
...
export async function translateJsonFile(opts: {
    ...
    try {
        ...
        onProgress(`[libretranslate] ⚠️  Translating to ${targetLang} (quality may be lower than Google Translate)...`);
        ...
        collectStrings(data);
        onProgress(`[libretranslate] Found ${entries.length} strings in base file`);
        ...
        onProgress(`[libretranslate] ${entriesToTranslate.length} properties need translation (new or blank)`);
        onProgress(`[libretranslate] ${entries.length - entriesToTranslate.length} properties already translated (preserved)`);

        if (entriesToTranslate.length === 0) {
            onProgress(`[libretranslate] ✓ All properties already translated, nothing to do`);
            return;
        } else {
            onProgress(`[libretranslate] First 3 strings to translate: ${entriesToTranslate.slice(0, 3).map(e => `${e.path.join('.')}="${e.value.substring(0, 30)}"`).join(', ')}`);
        }
        ...
        onProgress(`[libretranslate] Starting translation: source lang="${sourceLang}" (main: "${sourceLang?.split('-')[0]}"), target lang="${targetLang}"`);
            ...
            async (entry) => {
                try {
                    ...
                    if (completed <= 3 || completed === entriesToTranslate.length) {
                        ...
                        const marker = isSame ? "❌" : "✓";
                        onProgress(
                            `[libretranslate] ${marker} #${completed}: "${entry.value.substring(0, 25)}" -> "${result.substring(0, 25)}"`
                        );
                    } else if (completed % 10 === 0) {
                        const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
                        onProgress(
                            `[libretranslate] Progress: ${completed}/${entriesToTranslate.length} (${elapsed}s)`
                        );
                    }
                    return result;
                } catch (err) {
                    onProgress(`[libretranslate] Failed to translate "${entry.value}": ${err}`);
        ...
        onProgress(`[libretranslate] Merging ${translatedValues.length} new translations with existing data...`);
        ...
        onProgress(`[libretranslate] Applied ${appliedCount} out of ${entriesToTranslate.length} new translations (${entriesToTranslate.length - appliedCount} unchanged)`);

        if (appliedCount === 0) {
            onProgress(`[libretranslate] WARNING: No translations were different from original! Sample: "${entriesToTranslate[0]?.value}" is still same after translation.`);
        }

        const totalTime = ((Date.now() - startTime) / 1000).toFixed(1);
        onProgress(`[libretranslate] ✓ Completed ${entriesToTranslate.length} translations in ${totalTime}s`);

        await fs.writeFile(outputFile, JSON.stringify(mergedData, null, 2) + "\n", "utf8");
        onProgress(`[libretranslate] Saved: ${outputFile}`);
    } catch (err: any) {
        throw new Error(`LibreTranslate error: ${err.message}`);
    }
}
